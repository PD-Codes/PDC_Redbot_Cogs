...
| leveling | Alpha 0.1.0 | MEE6-style XP for messages + voice, role rewards, leaderboard, level-up announcements. Opt-in, DE/EN, dashboard. | `level show/leaderboard`, `levelset enable/xp/cooldown/voicexp/announce/reward/noxp/…` | pd-codes |
| tempvoice | Alpha 0.1.0 | Join-to-create temporary voice channels with owner controls (button panel + slash commands) and auto-cleanup. Opt-in, DE/EN, dashboard. | `tempvoice rename/limit/lock/hide/transfer/claim/kick`, `tempvoiceset creator/list/language` | pd-codes |
| tickets | Alpha 0.1.0 | Support tickets via panel button (threads or channels), claiming, transcripts (txt/HTML), dashboard ticket overview. Opt-in, DE/EN. | `ticket close/claim/reopen/delete/mine/list`, `ticketset panel/supportrole/mode/…` | pd-codes |
| starboard | Alpha 0.1.0 | Mirrors messages with enough star reactions into a star channel (live-updating embeds) + top leaderboard. Opt-in, DE/EN, dashboard. | `starboardtop`, `starboardset enable/channel/emoji/threshold/ignore/…` | pd-codes |
| suggestions | Alpha 0.1.0 | Numbered suggestions with vote reactions, optional mod-review queue, approve/deny/consider + DM notify. Opt-in, DE/EN, dashboard. | `suggest`, `suggestion approve/deny/consider/list`, `suggestset enable/channel/…` | pd-codes |
| livealerts | Alpha 0.1.0 | Twitch live & YouTube upload alerts with templates, role pings, offline handling (Twitch Helix, YouTube RSS). Opt-in, DE/EN, dashboard. | `livealerts enable/addtwitch/addyoutube/remove/list/message/ping/…` | pd-codes |
| automodplus | Alpha 0.1.0 | Anti-spam, anti-raid & regex filter — 8 rules, join-surge lockdown, per-rule actions/exemptions. Opt-in, DE/EN, dashboard. | `automodplus status/rule/regex/raid/lockdown/unlock/…` | pd-codes |
